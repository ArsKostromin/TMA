...
def parse_attributes(attributes):
    ...
    Возвращает: backdrop, model, pattern и редкости (permille), оригинальные детали.
    """
    result = {
        "backdrop_name": None,
        "model_name": None,
...
def extract_gift_data(action, message) -> dict:
    ...
    # ton_address - используется как уникальный идентификатор (Slug или ID)
    ton_address = getattr(gift_info, 'slug', None) or str(getattr(gift_info, 'id', ''))
    title = getattr(gift_info, 'title', 'Gift')
    slug = getattr(gift_info, 'slug', None) # Используется как Symbol
    
    # NOTE: В вашем списке есть поле "id". Если оно означает ID объекта
    # в Telegram, используйте getattr(gift_info, 'id', None). Если это
    # ID для Django, его добавлять здесь не нужно.
    ...
    # Цена в Toncoin (value_amount обычно в centi-ton, 100 centi-ton = 1 TON)
    price_ton = value_amount / 100 if value_amount else None
    
    # 3. Изображение
    image_url = None
    # ... (логика извлечения image_url без изменений)
    ...
    gift_data = {
        # Основные поля из вашего списка:
        ...
        # Поле "id" (если нужно, берем ID объекта Telegram)
        # Если в Django это поле auto-increment, удалите эту строку.
        # "id": gift_id_tg, 
        
        # Атрибуты (включая "backdrop")
...
async def handle_star_gift(message, client, **kwargs):
    ...
    gift_data.update({
        ...
        # ❌ УДАЛЕНО: "telegram_message_id": message.id
    })

    # 3. Логирование всех данных
